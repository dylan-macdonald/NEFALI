...
class Writer:
    ...
    Inject steering vectors during the forward pass to influence outputs.
    """
    ...
    def create_steering_from_concept(self,
                                      concept_name: str,
                                      strength: float = 1.0) -> SteeringConfig:
        """
    ...
    def create_steering_from_prompts(self,
                                      prompt_from: str,
                                      prompt_to: str,
                                      strength: float = 1.0,
                                      layer_idx: Optional[int] = None) -> SteeringConfig:
        """
    ...
    def create_steering_from_neuron(self,
                                     layer_idx: int,
                                     neuron_idx: int,
                                     value: float = 1.0) -> SteeringConfig:
        """
            ...
            if config.token_positions is None:
                ...
                for pos in config.token_positions:
                    if pos < hidden_states.shape[1]:
                        hidden_states[:, pos, :] = hidden_states[:, pos, :] + steering_tensor
    ...
    def sweep_steering_strength(self,
                                 model,
                                 tokenizer,
                                 prompt: str,
                                 config: SteeringConfig,
                                 strengths: List[float] = [-2.0, -1.0, 0.0, 1.0, 2.0],
                                 max_new_tokens: int = 50) -> Dict[float, str]:
        """
